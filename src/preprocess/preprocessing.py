...
class Dataset:
    ...
    def __getitem__(self, idx):
        ...
        def downscale(filename, change: bool = False):
            read    = cv2.imread(filename)
            resized = cv2.resize(read, (self.w, self.h))
            if change == True:
                flipped = cv2.flip(resized, 1) # Horizontal flip
                sharpening = np.array([[-1, -1, -1], [-1, 5, -1], [-1, -1, -1]])
                sharp = cv2.filter2D(flipped, -1, sharpening) # Adding sharpness
                resized = sharp
            return resized

        if "rev_" in str(data.filename):
            actual_filename = str(data.filename).replace("rev_", "")
            img = jnp.array(downscale(actual_filename, change = True), jnp.float32)
            
        else:
            img = jnp.array(downscale(data.filename, change = False), jnp.float32)
        img = img / 255.0
...
def get_loader(
    ...
    for file in tqdm(sorted(annotation_path.iterdir())):
        ...
        # Creating flipped coords for horizontal flip.
        rev_final_array = jnp.zeros(shape = (grid_size_y, grid_size_x, out_classes))
        for idx in range(len(minX)):
            rev_minX = (width - 1) - minX[idx]
            rev_maxX = (width - 1) - maxX[idx]
            cell, pos = create_label(
                rev_minX,
                rev_maxX,
                minY[idx],
            ...
            if cell is not None:
                rev_final_array = rev_final_array.at[pos].set(cell)
        
        rev_filepath = images_path / Path("rev_" + str(filename))
        df.add(rev_filepath, rev_final_array)
